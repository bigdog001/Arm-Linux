...
static int _wpa_ctrl_command(struct wpa_ctrl *ctrl, char *cmd, int print)
{
	...
	ret = wpa_ctrl_request(ctrl, cmd, os_strlen(cmd), ret_buf, &ret_len,
			       wpa_cli_msg_cb);
	if (ret == -2) {
	...
	if (print) {
		ret_buf[ret_len] = '\0';
		printf("%s", ret_buf);
		if (interactive && ret_len > 0 && ret_buf[ret_len - 1] != '\n')
			printf("\n");
	}
	return 0;
}
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        /* 连接该AP: add_network, set_network, select_network */
