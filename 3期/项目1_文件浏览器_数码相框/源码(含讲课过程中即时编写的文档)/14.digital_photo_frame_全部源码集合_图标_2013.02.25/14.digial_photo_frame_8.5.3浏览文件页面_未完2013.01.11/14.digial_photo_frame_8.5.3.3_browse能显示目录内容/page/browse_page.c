...
static int CalcBrowsePageDirAndFilesLayout(void)
{
    ...
    DBG_PRINTF("g_iDirFileNumPerRow = %d, g_iDirFileNumPerCol = %d\n", g_iDirFileNumPerRow, g_iDirFileNumPerCol);
    ...
    DBG_PRINTF("g_tBrowsePageDirAndFileLayout: \n");
    DBG_PRINTF("(%d, %d) ~ (%d, %d)\n", iTopLeftX, iTopLeftY, iBotRightX, iBotRightY);
...
static int GenerateBrowsePageDirAndFile(int iStartIndex, int iDirContentsNumber, PT_DirContent *aptDirContents, PT_VideoMem ptVideoMem)
{
    ...
    if (!bGetDirFileBmp)
    {
        ...
        DBG_PRINTF("tDirIconPixelDatas:    iHeight = %d, iWidth = %d, iLineBytes = %d, iTotalBytes = %d, aucPixelDatas = 0x%x\n", tDirIconPixelDatas.iHeight, tDirIconPixelDatas.iWidth, tDirIconPixelDatas.iLineBytes, tDirIconPixelDatas.iTotalBytes, tDirIconPixelDatas.aucPixelDatas);
        DBG_PRINTF("tOriginIconPixelDatas: iHeight = %d, iWidth = %d, iLineBytes = %d, iTotalBytes = %d, aucPixelDatas = 0x%x\n", tOriginIconPixelDatas.iHeight, tOriginIconPixelDatas.iWidth, tOriginIconPixelDatas.iLineBytes, tOriginIconPixelDatas.iTotalBytes, tOriginIconPixelDatas.aucPixelDatas);
        DBG_PRINTF("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
...
static void BrowsePageRun(void)
{
    ...
    DBG_PRINTF("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
    DBG_PRINTF("g_iDirContentsNumber = %d\n", g_iDirContentsNumber);
