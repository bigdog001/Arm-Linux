	...
	unsigned int value)
{
    ...
    printk("wm8976_write : reg = %d, 0x%x, val = %d, 0x%x\n", reg, reg, value, value);
...
static int snd_soc_wm8976_probe(struct snd_soc_codec *codec)
{
    ...
    *gpbcon |= ((1<<4) | (1<<6) | (1<<8));
	...
	/* software reset */
	wm8976_write(codec, 0, 0);

	/* OUT2的左/右声道打开
	 * 左/右通道输出混音打开
	 * 左/右DAC打开
	 */
	wm8976_write(codec, 0x3, 0x6f);
	
	wm8976_write(codec, 0x1, 0x1f);//biasen,BUFIOEN.VMIDSEL=11b  
	wm8976_write(codec, 0x2, 0x185);//ROUT1EN LOUT1EN, inpu PGA enable ,ADC enable

	wm8976_write(codec, 0x6, 0x0);//SYSCLK=MCLK  
	wm8976_write(codec, 0x4, 0x10);//16bit 		
	wm8976_write(codec, 0x2B,0x10);//BTL OUTPUT  
	wm8976_write(codec, 0x9, 0x50);//Jack detect enable  
	wm8976_write(codec, 0xD, 0x21);//Jack detect  
	wm8976_write(codec, 0x7, 0x01);//Jack detect 
